//const char* ssid = "NET 301"; //VARIÁVEL QUE ARMAZENA O NOME DA REDE SEM FIO EM QUE VAI CONECTAR
//const char* password = "surigatao1310"; //VARIÁVEL QUE ARMAZENA A SENHA DA REDE SEM FIO EM QUE VAI CONECTAR
...
void setup_wifi() {
  delay(500); //INTERVALO DE 10 MILISEGUNDOS
...
//  WiFi.config(ip, gateway, subnet); //PASSA OS PARÂMETROS PARA A FUNÇÃO QUE VAI SETAR O IP FIXO NO NODEMCU
  ...
  Serial.println(""); //PULA UMA LINHA NA JANELA SERIAL
  Serial.println("Conectado a rede sem fio "); //ESCREVE O TEXTO NA SERIAL
...
void loop_wifi() {
  ...
  client.println("Estado do led: " + stateLed);
  delay(1); //INTERVALO DE 1 MILISEGUNDO
  Serial.println("Cliente desconectado"); //ESCREVE O TEXTO NA SERIAL
  Serial.println(""); //PULA UMA LINHA NA JANELA SERIAL

  client.println(request);
  if (request.indexOf("LIGADO") != -1) {
    client.println("ENTROU LIGADO");
    digitalWrite(LED, HIGH);
    digitalWrite(RELE, LOW);
  }
  if (request.indexOf("DESLIGADO") != -1) {
    client.println("ENTROU DESLIGADO");
